...
void PinataClient::dilithiumSetPublicPrivateKeyPair(
...
void PinataClient::dilithiumSign(
    const uint8_t* messageBuffer,
    size_t messageBufferSize,
    uint8_t* signedMessageBuffer,
...
bool PinataClient::dilithiumVerify(
...
void PinataClient::kyber512SetPublicPrivateKeyPair(
    const uint8_t* publicKey,
    size_t publicKeySize,
    const uint8_t* privateKey,
    size_t privateKeySize)
...
void PinataClient::kyber512Generate(
    uint8_t* sharedSecretBuffer,
    size_t sharedSecretBufferSize,
    uint8_t* keyEncapsulationMessageBuffer,
    size_t keyEncapsulationMessageBufferSize)
{
...
void PinataClient::kyber512Decode(
    const uint8_t* keyEncapsulationMessageBuffer,
    size_t keyEncapsulationMessageBufferSize,
    uint8_t* sharedSecretBuffer,
    size_t sharedSecretBufferSize)
{
...
void PinataClient::doSymmetricCipherRequest(const uint8_t cmd, const uint8_t* input, const size_t inputSize, uint8_t* output, const size_t outputSize) {
    command(cmd);
...
void PinataClient::command(uint8_t cmd) {
    boost::asio::write(
        ...
        boost::asio::transfer_at_least(sizeof(cmd))
    );
}

void PinataClient::read(uint8_t *data, size_t size) {
    boost::system::error_code ec;
    // set up a deadline timer for a 1-second timeout
