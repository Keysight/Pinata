...
//Training board trigger functions: trigger for cryptographic operations set in PC2
void set_trigger(){GPIOC->BSRRL = GPIO_Pin_2;} // Trigger goes high in pin PC2
void clear_trigger(){GPIOC->BSRRH = GPIO_Pin_2;} // Trigger goes low in pin PC2
...
/**
  * @brief  Sets the selected data port bits.(voltage=3.3V)
...
/**
  * @brief  Clears the selected data port bits (voltage=0V).
  * @note   This functions uses GPIOx_BSRR register to allow atomic read/modify 
  *         accesses. In this way, there is no risk of an IRQ occurring between
  *         the read and the modify access.
  * @param  GPIOx: where x can be (A..I) to select the GPIO peripheral.
