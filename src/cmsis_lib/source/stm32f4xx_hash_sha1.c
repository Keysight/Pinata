...
/**
  ...
  * @param  iterations: of feeding back the output into next SHA1
  * @retval An ErrorStatus enumeration value:
  *          - SUCCESS: digest computation done
  *          - ERROR: digest computation failed
  */
ErrorStatus HASH_SHA1(uint8_t *Input, uint32_t Ilen, uint8_t Output[20], uint32_t iterations)
{
  ...
  uint32_t inputaddrbase  = inputaddr;
  uint32_t outputaddrbase = outputaddr;
  ...
  for (; iterations > 0; iterations--) 
  {
    ...
    Ilen = 20;
    inputaddr = outputaddrbase;
    outputaddr = outputaddrbase;
...
ErrorStatus HMAC_SHA1(uint8_t *Key, uint32_t Keylen, uint8_t *Input,
                      uint32_t Ilen, uint8_t Output[20], uint32_t iterations)
{
  HASH_InitTypeDef SHA1_HASH_InitStructure;
  HASH_MsgDigest SHA1_MessageDigest;
  __IO uint16_t nbvalidbitsdata = 0;
