...
// Note: function swapPointersConditionally() that is called when this is defined
// was commented out in the original source code.
...
void nop_block(void)
{
//#define NOP_BLOCK_AS_LOOP
//#define NOP_BLOCK_AS_LOOP_2
#define NOP_BLOCK_UNROLLED_LOOP

#ifdef NOP_BLOCK_AS_LOOP
	const int N_NOPs = 1000;
	int i;
	for (i = 0; i < N_NOPs; i++)
	{
		asm("mov	r0, r0");
	}
#elif defined(NOP_BLOCK_AS_LOOP_2)
	const int N_NOPs = 1000;
	int i;
	for (i = 0; i < N_NOPs; i++)
	{
...
#elif defined(NOP_BLOCK_UNROLLED_LOOP)
	#include "utils/nop_block.h"

#elif defined(NOP_BLOCK_NONE)

#else
	#error "nop block method not selected"
#endif
...
void inflate_leakage(uint8_t swapbit)
{
	...
	for (i = 0; i < N_ITERS; i++)
	{
		// use volatile to ensure this ASM block will not be optimized out
		asm volatile ("and %[a], %[b]" : [a] "=r" (swapbit) : [b] "r" (swapbit));
	}
}

#else

//#pragma GCC push_options  //error: not supported for platform
//#pragma GCC optimize ("O0")
void inflate_leakage(u8 swapbit, u8 xor_mask)
{
	const int N_ITERS = 1000;
	int i;

	u8 s = swapbit;
	volatile u8 m = xor_mask;

	for (i = 0; i < N_ITERS; i++)
	{
		asm volatile ("and %[a], %[b]" : [a] "=r" (s) : [b] "r" (m));
	}
}
...
#if 0	// Original cswap code
        curve25519_cswap(&state, swap);
#endif

#if 0	// cswap surrounded by nops
        nop_block();
        curve25519_cswap(&state, swap);
        nop_block();
#endif

#if 1	// cswap surrounded by nops and increased leakage
        nop_block();
        inflate_leakage(swap);  //inflate_leakage(swap, 0);
//        inflate_leakage(swap);
//        inflate_leakage(swap);
        nop_block();
        curve25519_cswap(&state, swap);
        nop_block();
...
#define LEN_A_IN_32_LIMBS	8
#define LEN_B_IN_32_LIMBS	1
#define LEN_R_IN_32_LIMBS	(LEN_A_IN_32_LIMBS + LEN_B_IN_32_LIMBS)

__attribute__((optimize("unroll-loops")))
void mul_256_32(uint32_t R[LEN_R_IN_32_LIMBS], uint32_t A[LEN_A_IN_32_LIMBS], uint32_t B /*[LEN_B_IN_32_LIMBS]*/ )
{
    uint32_t c = 0;
    uint64_t t, bb, aa;
    uint32_t L, U;
...
//    for (j = 0; j < LEN_B_IN_32_LIMBS; j++) {
    	c = 0;
		for (i = 0; i < LEN_A_IN_32_LIMBS; i++)
		{
			bb = B;  //B[j];
			aa = A[i];
			t = bb * aa + (uint64_t) c + (uint64_t) R[i + j];
			L = (uint32_t)( ((uint32_t) t) & 0x00000000FFFFFFFF );
			U = (uint32_t)( (t >> 32) & (uint32_t) 0x00000000FFFFFFFF );
			R[i + j] = L;
			c = U;
		}
		R[LEN_A_IN_32_LIMBS + j] = c;
    //}
}

void fe25519_mul_256_32(fe25519 *r, fe25519 *a, uint32_t b)
{
	UN_512bitValue R;
	int i;
	for(i = 0; i < 16; i++) R.as_uint32[i] = 0;

#if 1
	mul_256_32(R.as_uint32, a->as_uint32, b);
	fe25519_reduceTo256Bits_asm(r, &R); // then reduce R
#else
	//memcpy(r, a, sizeof(*a)); //TODO: DEBUG, REMOVE LATER
	fe25519 t = *a;
	*r = t;
#endif
}
...
int
crypto_scalarmult_curve25519_rand_proj_coords(
	unsigned char *r,
	const unsigned char *s,
	const unsigned char *p
)
{
...
#endif
    {
        ...
        //========================================================
        // Re-randomize the coordinates (Xp,Zp) and (Xq,Zq)
        //========================================================
        uint32_t lambda_rand32;
        prng_get_bytes(&g_prng_ctx, (uint8_t *)&lambda_rand32, sizeof(lambda_rand32));
        lambda_rand32 |= 1;  //force the first bit to 1, thus avoiding a multiplication by zero when lambda_rand32 == 0.

#if 0   // Use the 256 x 256 bit multiplier
        fe25519 lambda;
        fe25519_setzero(&lambda);
        lambda.as_uint32[0] = lambda_rand32;
        fe25519_mul(&state.xp, &state.xp, &lambda);
        fe25519_mul(&state.zp, &state.zp, &lambda);
        fe25519_mul(&state.xq, &state.xq, &lambda);
        fe25519_mul(&state.zq, &state.zq, &lambda);

#else  // Use a 256 x 32 bit multiplier
        fe25519_mul_256_32(&state.xp, &state.xp, lambda_rand32);
        fe25519_mul_256_32(&state.zp, &state.zp, lambda_rand32);
        fe25519_mul_256_32(&state.xq, &state.xq, lambda_rand32);
        fe25519_mul_256_32(&state.zq, &state.zq, lambda_rand32);
#endif
